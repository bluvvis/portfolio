...
export function initSphere(stage) {
  ...
  const skills = source.map(item => ({
    name: item.dataset.skill, category: item.dataset.category,
    description: item.dataset.description, projects: item.dataset.projects.split(' ')
  }));
  if (!skills.length) throw new Error('Skill list is empty');
  const categoryNames = { frontend: 'Frontend', backend: 'Backend', ml: 'ML / Data', delivery: 'Delivery / Tests' };
  const palette = { frontend: 0xff80b9, backend: 0x9bc7fb, ml: 0xc0a5ff, delivery: 0xa6d5ba };
  ...
  function select(index, announce = false) {
    ...
    if (changed && !motion.matches && !globalPaused && detail.animate) {
      ...
      detailAnimation = detail.animate([{ opacity: 0.65, transform: 'translateY(6px)' }, { opacity: 1, transform: 'translateY(0)' }], { duration: 240, easing: 'ease-out' });
    }
    document.querySelector('#skillDetailCategory').textContent = categoryNames[skill.category];
    ...
    nodes.forEach((node, i) => {
      ...
      node.button.classList.toggle('is-related', i !== index && skills[i].category === skill.category);
    });
    lines.forEach(line => {
      const active = line.userData.a === index || line.userData.b === index;
      line.material.opacity = active ? 0.46 : 0.07;
    });
    if (announce) document.querySelector('#skillAnnouncement').textContent = `${skill.name}. ${skill.description} Связанные проекты показаны в панели.`;
    requestFrame();
  }

  function updatePause() {
    if (motion.matches || globalPaused) detailAnimation?.cancel();
    pause.disabled = motion.matches || globalPaused;
    const stopped = userPaused || motion.matches || globalPaused;
    pause.setAttribute('aria-pressed', String(stopped));
    pause.textContent = motion.matches || globalPaused ? 'Анимация отключена' : stopped ? 'Продолжить вращение ▷' : 'Пауза вращения Ⅱ';
    lastTime = 0;
    requestFrame();
  }
  ...
  function render(now) {
    ...
    const automatic = !userPaused && !globalPaused && !motion.matches && !hovered && !focused && !dragging;
    ...
    nodes.forEach(node => {
      ...
      node.button.style.setProperty('--depth-scale', String(0.9 + smoothDepth * 0.16));
      node.button.style.setProperty('--depth-opacity', String(0.42 + smoothDepth * 0.58));
    });
    try {
  ...
  try {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      particles.push(2 * Math.sin(phi) * Math.cos(theta), 2 * Math.cos(phi), 2 * Math.sin(phi) * Math.sin(theta));
    }
    const geometry = new THREE.BufferGeometry();
    geometry.setAttribute('position', new THREE.Float32BufferAttribute(particles, 3));
    sphere.add(new THREE.Points(geometry, new THREE.PointsMaterial({ color: 0xe7e5de, size: 0.018, transparent: true, opacity: 0.3 })));
    [0, Math.PI / 2].forEach(angle => {
      const ring = new THREE.Mesh(new THREE.TorusGeometry(2.04, 0.0025, 5, 96), new THREE.MeshBasicMaterial({ color: 0xe7e5de, transparent: true, opacity: 0.15 }));
      ring.rotation.x = angle;
      sphere.add(ring);
    });
    sphere.add(new THREE.Mesh(new THREE.SphereGeometry(0.035, 10, 10), new THREE.MeshBasicMaterial({ color: 0xff2d8d })));

    skills.forEach((skill, i) => {
      const phi = Math.acos(1 - 2 * (i + 0.5) / skills.length);
      const theta = Math.PI * (3 - Math.sqrt(5)) * i;
      const position = new THREE.Vector3(2 * Math.sin(phi) * Math.cos(theta), 2 * Math.cos(phi), 2 * Math.sin(phi) * Math.sin(theta));
      ...
      button.setAttribute('aria-label', `${skill.name} — показать опыт и проекты`);
      ...
      listen(button, 'pointerenter', event => { if (event.pointerType === 'mouse') { hovered = true; requestFrame(); } });
      listen(button, 'pointerleave', () => { hovered = false; lastTime = 0; requestFrame(); });
    ...
    const relations = [['React', 'TypeScript'], ['React', 'FastAPI'], ['FastAPI', 'Python'], ['FastAPI', 'PostgreSQL'], ['PostgreSQL', 'SQL / SQLAlchemy'], ['Python', 'scikit-learn'], ['scikit-learn', 'pandas'], ['Python', 'NLP / n-grams'], ['Python', 'Transformers'], ['PySpark', 'Cassandra'], ['Docker', 'Kubernetes'], ['Kubernetes', 'Helm'], ['FastAPI', 'pytest'], ['React', 'Vitest']];
    relations.forEach(([a, b]) => {
      const ai = skills.findIndex(skill => skill.name === a);
      const bi = skills.findIndex(skill => skill.name === b);
      if (ai < 0 || bi < 0) return;
      const line = new THREE.Line(new THREE.BufferGeometry().setFromPoints([nodes[ai].position, nodes[bi].position]), new THREE.LineBasicMaterial({ color: palette[skills[ai].category], transparent: true, opacity: 0.07 }));
      line.userData = { a: ai, b: bi };
      sphere.add(line);
      lines.push(line);
    });
    listen(pause, 'click', () => { userPaused = !userPaused; updatePause(); });
    listen(motion, 'change', updatePause);
    listen(document, 'portfolio:motion-change', () => {
      globalPaused = document.documentElement.dataset.motion === 'paused';
      updatePause();
    });
    const holdsRotation = element => element === stage || Boolean(element?.closest?.('.skill-node-button'));
    listen(stage, 'focusin', event => { focused = holdsRotation(event.target); requestFrame(); });
    listen(stage, 'focusout', event => { focused = holdsRotation(event.relatedTarget); lastTime = 0; requestFrame(); });
    listen(stage, 'keydown', event => {
      if (event.target !== stage || !['ArrowLeft', 'ArrowRight', 'Home'].includes(event.key)) return;
      event.preventDefault();
      horizontalAngle = event.key === 'Home' ? -0.3 : horizontalAngle + (event.key === 'ArrowRight' ? 0.18 : -0.18);
      requestFrame();
    });
    ...
    listen(renderer.domElement, 'webglcontextlost', event => {
      event.preventDefault();
      dispose('3D-карта остановлена. Все навыки и проекты доступны в списке ниже.');
    });
    ...
    dispose('3D-карта недоступна. Все технологии и ссылки на проекты есть в списке ниже.');
    throw error;
  }
}
